...
-   What is the mean departure delay time, among available values?
    -   The mean departure delay time is 12.6 minutes among non-NA values.
...
-   Which carriers are at the top of both `mean` and `median` departure delay?
    -   FL and F9 are both near the top of mean and median delays. WN also floats somewhere near the top of both.
-   Which carriers differ in the two lists?
    -   EV and YV have high mean delays, but doesn't show up in the median delay.
-   What do negative values of `dep_delay` mean?
    -   Presumably, these flights left early before their scheduled time.
...
test_iqr <-
  quantile(v_test_iqr, probs = c(0.75), na.rm = TRUE) - 
...
-   Which carriers are at the top of both `sd` and `IQR` departure delay?
    -   F9, FL, YV, and EV are on both lists.
-   Which carriers differ in the two lists?
    -   While HA has a very high standard deviation, it has a fairly low IQR.
    -   9E has a fairly high IQR, but comparatively lower sd.
-   Which carrier has the biggest difference between its `sd` and `IQR`? What might this mean?
    -   HA has a huge sd, but pretty low iqr. This variance suggests that HA has some outlier fights that were delayed by a large amount, but that in general, there was not much variation in the timeliness of their flights.
...
-   What do you predict the correlations to be?
    -   I perdict the Pearson correlation will be close to zero as the negative and positive slops will cancel each other out.
    -   The Spearman correlation will be higher and recognize?
-   What are the actual correlations?
    -   Actual are both very close to zero.
