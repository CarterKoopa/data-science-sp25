...
-   The true difference between the treatment mean and control mean is such that the treatment yield should be `1` unit higher. How does your difference compare with that difference?
    -   My difference does not correlate to this, as treatement is actually about 0.35 units lower.
...
-   Which experimental condition do your results suggest has a higher mean yield? The Treatment `T` or Control `C`?
    -   This result suggests that the Treatment `T` has a higher mean.
-   The true difference between the treatment mean and control mean is such that the treatment yield should be `1` unit higher. How does your difference compare with that difference?
    -   My difference between the `T` and `C` plots is slightly lower at about 0.8 higher for the `T`. However, this does have the correct sign.
-   What effects are you seeing? The effect of the Treatment `T` *only*, or the combined effects of the Treatment and the river? How does this affect your estimate of the effect of the treatment?
    -   This data contains the combined effect of both the treatment and the proximity to the river such that we are not getting a singular, true value of the influence of the fertilizer.
...
-   Which experimental condition do you find has a higher mean yield? The Treatment `T` or Control `C`?
    -   Here, treatment `T` plots have a higher mean yield.
-   Since you've randomized the conditions, will the confound (river) have a *consistent* effect on the outcome?
    -   On average, across samples, the river can not have a consistent impact biased towards either the control or treatment.
-   The true difference between the treatment mean and control mean is one unit. How does your difference compare with that value?
    -   My value is slightly lower with the `T` plots producing about 0.6 units higher mean yield. This is smaller than the true value but lower than the estimated value with a fixed plot order.
