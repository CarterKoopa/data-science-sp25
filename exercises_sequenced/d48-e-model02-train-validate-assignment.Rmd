...
-   Which model---`fit_d2` or `fit_over`---has lower error on `df_measurements`?
    -   `fit_d2` is more accurate
-   Based *solely* on these results, which hyperparameter value (polynomial order) would you choose?
    -   I would chose a lower hyperparameter (in this case, degree 2). Ideally, something in the middle would be chosen, however.
-   Would that be a good choice? Why or why not?
    -   I mean given only these options, it would be a good choice. However, the model is still extremely inaccurate and would not result in any useable data.
...
-   Based on the `Train` error, which polynomial order would you pick?
    -   Training error is minimized at quite a high order polynomial - although not on the graph, a value as high as 8 would make sense.
-   Based on the `Validate` error, which polynomial order would you pick?
    -   The error starts to increase again after an order of 3 or 4.
-   Which selection leads to the correct choice?
    -   The selection should be based off the validation error since this better represents the entire dataset.
