...
*Purpose*: Often, our data do not include all the facts that are xrelevant to the decision we are trying to make. Statistics is the science of determining the conclusions we can confidently make, based on our available data. To make sense of this, we need to understand the distinction between a *sample* and a *population*, and how this distinction leads to *estimation*.
...
-   (Describe the shape of the sampling distribution)
    -   The sampling distribution follows a vaugly similar bell curve shape.
...
se_q2.1 <- # Population: norm(mean = 2, sd = 4), observations: n = 16
  4 / sqrt(16)
se_q2.2 <- # Population: norm(mean =-2, sd = 8), observations: n = 32
  8 / sqrt(32)
