...
-   Which `class` has the most vehicles?
    -   `SUV` has the most vehicles - this is easiest to decipher from the frequency polygon plot.
-   Which `class` has the broadest distribution of `cty` values?
    -   `Subcompact`s seem to have the broadest distribution of `cty` values. This is easiest to decipher from the density plot - here, `subcompact`s have the most spread out area under the curve.
-   Which graph---`vis1` or `vis2`---best helps you answer each question?
    -   See the answer to each question above.
...
-   Binwidth 0.01 - peaks at 0.3, 0.5, 0.7, 0.9, 1. Each peak results in an expontential decay.

-   Binwidth 0.025 - peaks 0.3 (the largest), 0.5, 0.7, 1 (2nd largest). The bin at 0.9 is still there, but a good bit smaller.

-   Binwidth = 0.05 - peak at 0.3 continues to become more apparent. Still a secondary peak at 1, 0.9 is mostly gone and 0.5 and 0.7 are still relevant.

-   Binwith = 0.1 - Major peak at 0.3. Still a peak at 1.
