...
Using `group_by` first results in ummarize applying across the dimensions summerized in the `group_by`, rather than just as an entire group.
...
-   Comment on the relative values of `cov_depth` and `cov_diff`.
    -   `cov_depth` is very small relative to the `mean_depth` - several orders of magnitude smaller.
    -   `cov_diff` is HUGE compared to the mean value of `diff` - several orders of magnitude larger. This suggests there is some huge outliers in this dataset.
...
-   The higher quality the cut, generally, the larger the variation in the price. However, there is no clear relation between the quality of the cut and the mean price or standard deviation.
