...
-   What percent of homes are *occupied*?
    -   If 17 homes are occupied, 537 homes are unoccupied. This mean 96.9% (97% rounded) of homes are occupied.
-   Are there any sources of *real* uncertainty in the percent occupied you calculated?
    -   While I don't believe there is real uncertainty at the moment of calculation - houses are either occupied or are not - the islands is a dynamic situation where islanders can move at will. As such, it is quite possible that by the time I finished counting, or several days later, islanders have moved and introduced real uncertainty into the calculated percent occupied.
-   Are there any sources of *erroneous* uncertainty in the percent occupied you calculated?
    -   Given that I counted all of the houses manually, it is possible that I counted wrong, which introduces erroneous uncertainty. However, I counted multiple times and compared with multiple people in an attempt to reduce this uncertainty as much as possible.
...
-   Is this sample representative of *all* houses in Helvig? Why or why not?
    -   It is possible that this sample is representative. However, I think this is unlikely given it is not a random sample of the entire population, therefore reducing whatever biases would cause people to live in the *first* 25 houses in the town. Whether or not this sample is representative could be uncovered by comparing the standard error and underlying distributions of the population and the sample.
...
-   Which sample---sequential or random---is more *representative* of all homes Helvig? Why?
    ...
    -   The random sample has an equal chance of all members of the population being selected. The same is not true for the sequential choice, where any number of factors could influence who ends up in the first 25 houses. The first 25 houses are not guaranteed to represent the entire population accurately.
...
-   Did you find any highly prevalent names using `df_sample_random` that you *didn't* find in q3 (using `df_sample_seq`)? Write them here.
    -   Collins, the most popular name in the random sample, did not appear at all in the sequential sample. Regan, the third most popular, is the same. However, number of occurrences for the most common last names was lower in the random sample compared to the sequential sample.
-   Is there any reason that people with the same last name might tend to *live near each other*?
    -   The houses close to each other are more likely to contain family members who are hoping to stay close to each other. This is one reason why a sequential sample is potentially not representative of the population.
...
-   Note that we used the *random* sample with the bootstrap procedure in this task. Could we use the bootstrap to make a confidence interval using the sequential sample (`df_sample_seq`) that would be representative of all of Helvig? Why or why not?
    -   No
    -   Ultimately, however you mutate this sample data, you can not remove the in-built bias from having collected biased data in the first place. The influence of all houses being close together (such as having families live closer to each other) will always be present in the data.
...
-   We'll be studying the residents of a single town, Arcadia. Any adult residents from this town are part of the population we're interested in. We're interested in Arcadia in particular because it is a college town.
...
-   (What steps will you take to collect the data?)
    ...
    -   Have islanders take a follow-up IQ test after some amount of time (researched needed into the half-life of alcohol in The Islands time).
    -   Possibly rinse and repreat; this will need to be flushed out depending on how long the process ends up taking in our inital tests.
-   (How will you ensure the data is representative of your chosen population?)
    -   We will randomly select houses in Arcadia and look deeper at the mean statics of our sample relative to the population.
-   (For experiments only: How will you ensure any effects you observe are due to the treatment, and not due to other factors?)
    -   We can use the chat function to investigate the mood, behaviors, etc of the islanders on the day of the study to determine if we believe other variables are at play. Further, we can ensure nothing happens in the interm between the first and second IQ test except for our actions.
-   (How will you choose your sample size?)
    ...
    -   We're hoping to begin with 30 islanders (10 each). While this is a small sample, it is a starting point and will allow us to gauge how long that conducting this (expected to be) fairly labor intensive study is.
