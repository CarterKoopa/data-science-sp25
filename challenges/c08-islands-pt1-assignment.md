...
- Are there any sources of *erroneous* uncertainty in the percent
  ...
  - Given that I counted all of the houses manually, it is possible that
    I counted wrong, which introduces erroneous uncertainty. However, I
    counted multiple times and compared with multiple people in an
    attempt to reduce this uncertainty as much as possible.
...
- What last name is most prevalent in `df_sample_seq`?
  - The last name Sorensen is most prevalent in df_sample_seq.
- Is this sample representative of *all* houses in Helvig? Why or why
  not?
  - It is possible that this sample is representative. However, I think
    this is unlikely given it is not a random sample of the entire
    population, therefore reducing whatever biases would cause people to
    live in the *first* 25 houses in the town. Whether or not this
    sample is representative could be uncovered by comparing the
    standard error and underlying distributions of the population and
    the sample.
...
- Which sample—sequential or random—is more *representative* of all
  homes Helvig? Why?
  - The random is more representative.
  - The random sample has an equal chance of all members of the
    population being selected. The same is not true for the sequential
    choice, where any number of factors could influence who ends up in
    the first 25 houses. The first 25 houses are not guaranteed to
...
- Did you find any highly prevalent names using `df_sample_random` that
  ...
  - Collins, the most popular name in the random sample, did not appear
    at all in the sequential sample. Regan, the third most popular, is
    the same. However, number of occurrences for the most common last
    names was lower in the random sample compared to the sequential
    sample.
- Is there any reason that people with the same last name might tend to
  *live near each other*?
  - The houses close to each other are more likely to contain family
    members who are hoping to stay close to each other. This is one
    reason why a sequential sample is potentially not representative of
    the population.
...
- Note that we used the *random* sample with the bootstrap procedure in
  ...
  - Ultimately, however you mutate this sample data, you can not remove
    ...
    having families live closer to each other) will always be present in
    the data.
...
- (What population are you going to study?)
  - All of the Islanders? Only the residents of Helvig? Participants
    ...
    *covariates*)?
- We’ll be studying the residents of a single town, Arcadia. Any adult
  residents from this town are part of the population we’re interested
  in. We’re interested in Arcadia in particular because it is a college
  town.
...
- Age, upbringing, nutrition, drug use
...
- What impact does drug use/smoking have on IQ?
...
- (What steps will you take to collect the data?)
  ...
  - Have islanders take a baseline IQ test.
  - Have islanders drink alcohol.
  - Have islanders take a follow-up IQ test after some amount of time
    (researched needed into the half-life of alcohol in The Islands
    time).
  - Possibly rinse and repreat; this will need to be flushed out
    depending on how long the process ends up taking in our inital
    tests.
- (How will you ensure the data is representative of your chosen
  population?)
  - We will randomly select houses in Arcadia and look deeper at the
    mean statics of our sample relative to the population.
- (For experiments only: How will you ensure any effects you observe are
  due to the treatment, and not due to other factors?)
  - We can use the chat function to investigate the mood, behaviors, etc
    of the islanders on the day of the study to determine if we believe
    other variables are at play. Further, we can ensure nothing happens
    in the interm between the first and second IQ test except for our
    actions.
- (How will you choose your sample size?)
  ...
  - We’re hoping to begin with 30 islanders (10 each). While this is a
    small sample, it is a starting point and will allow us to gauge how
    long that conducting this (expected to be) fairly labor intensive
    study is.
