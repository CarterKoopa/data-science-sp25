...
- Given that Michelson’s study was conducted in 1880, it can be assumed
  that many more quantities were computed by hand than with a single
  line of R code. Given that all 100 velocities had to be computed and
  then averaged, it is reasonable to assume that perhaps this process to
  average them wasn’t computed with the same precision as is
  instantaneously computed by the computer. It can be assumed that these
  values were likely rounded at some point in the calculation.
...
- A majority of the daily averages are within the margin of error for
  both graphs. Both graphs have only 2-3 days (out of 18 days) where the
  mean is outside the margin of error.

- There are pretty large variations in the precision from day-to-day for
  both graphs. Some days have fairly high precision while other days
  have fairly low precision. The level of precision doesn’t necessarily
  correlate to the accuracy of the measurements.
...
![](c02-michelson-assignment_files/figure-gfm/q5-2-1.png)<!-- -->
...
- As shown in this graph, the vast majority of his measurements were
  ...
  added by Michaelson to account for his measurements being conducted in
  air, further pressing the measurements too high.
