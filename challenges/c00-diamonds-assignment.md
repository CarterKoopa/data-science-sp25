...
- In general, `price` increases with increasing `carat`
- There are several outliers at large carat values, alongside a fairly
  large variation within carat.
- There seems to be somewhat of an upper limit on price for each
  truncated integer carat value; for example, there is a huge jump
  upwards in the distribution of price from `0.9` to `1` carat.
...
- `price` increases with higher quality `cut`, with this different
  accounting for some of the price variation within equal `carat`
  - `Ideal` diamonds tend to fetch the highest price, while `fair` are
    the lowest.
  - There is a fairly large variation among `premium` and higher quality
    diamonds, while `fair` diamonds are more grouped at lower price.
  - Most diamonds are spread somewhere between 1 and 2 carats.
