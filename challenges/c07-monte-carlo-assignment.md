...
assertthat::assert_that(
   tibble(x = 0.25, y = 0.25) %>% # Pick a point *inside* the circle
...
- You chose a correct value of `stat(x, y)` when `x, y` is *outside* the
  ...
  - The area when `x, y` is outside of the circle, this point should not
    contribute to our pi estimate, so it should be equal to zero.
- You chose a correct value of `stat(x, y)` when `x, y` is *inside* the
  circle. Why did you choose this value?
  - This value, when `x, y` is inside the circle, should be equal to
    four. Normally, it would be equal to a 1 boolean value, but this
    probability needs to be multiplied by four since this square is only
    testing a quarter of the circle (this is not applicable to the area
    outside since it is already zero).
